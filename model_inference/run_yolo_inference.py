...
def _resolve_model_path(model_path: str) -> str | None:
    ...
    if os.path.exists(default):
        logging.info(f"Model not found at provided path. Using default model at {default}")
        return default
    logging.error(f"Model not found. Checked: '{model_path}' and '{default}'")
    return None


def _load_gaze_log_safe(gaze_log_path: str, fallback_path: str) -> pd.DataFrame | None:
    for path in [gaze_log_path, fallback_path]:
        if path and os.path.exists(path):
            try:
                return load_gaze_log(path)
            except Exception as e:
                logging.warning(f"Failed to load gaze log '{path}': {e}")
    return None
...
def process_video(
    cap, model, input_log, gaze_log=None, threshold=0.7
):
    """
...
def run_yolo_inference(
    ...
    if input_log.empty:
        logging.error(f"Input log '{input_log_path}' contains no mouse_click events.")
        return {"error": "Input log contains no mouse_click events. Cannot anchor video timestamps."}


    gaze_log = _load_gaze_log_safe(gaze_log_path, input_log_path)
...
def batch_process_video(
    cap, model, input_log, gaze_log=None, threshold=0.7, batch_size=16
):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logging.info(f"Using device: {device}")
    ...
    def process_results(results_batch):
        ...
        for i, results in enumerate(results_batch):
            # Reset per-frame transient values at the top of every frame
            click_time_ms = click_rt_ms = gaze_time_ms = gaze_rt_ms = None
...
def run_yolo_batch_inference(
    ...
    reaction_data = batch_process_video(
        cap,
        model,
        input_log,
        gaze_log,
